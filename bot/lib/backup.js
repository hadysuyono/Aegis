// Backup harian — snapshot memory + reminders ke folder 06-ARCHIVE/backup/YYYY-MM-DD/
// Sehingga kalau ada data corrupt, bisa restore manual dari snapshot.
...
export const dailySnapshot = async () => {
  ...
  for (const path of FILES) {
    ...
    await writeFile(archivePath, JSON.stringify(data, null, 2) + "\n", `backup: ${date} ${path.split("/").pop()}`);
